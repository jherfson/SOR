...
SUBROUTINE FIND_ENTROPY(vec,entropy,conf)
...
CHARACTER(10), 	                       INTENT(OUT)   :: entropy
...
ELSE
	temp = vec(rec+3)
	entropy = FIELD(temp,3,' ')
	IF(entropy(2:2).EQ.'-') entropy = 'not_found'
END IF	
