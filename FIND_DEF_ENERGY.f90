...
SUBROUTINE FIND_DEF_ENERGY(vec,def_energy,conf)
...
CHARACTER(20), 	                       INTENT(OUT)   :: def_energy
...
CALL SEARCH_IN_VECT(vec,'Final defect energy  =',rec,temp,c_start)
...
IF((rec.EQ.0).OR.(rec.GT.c_end))THEN
	def_energy = 'not_found'
ELSE
	def_energy = FIELD(temp,5,' ')
END IF	
...
MODULE FIND_DEF_ENERGY__MODULE
